...
def test_manual_save_revision_and_restore(tmp_path) -> None:
    ...
    assert restored.json()["current"]["kind"] == "generated"
    assert restored.json()["current"]["sections"]["completed"] == ["完成日报接口"]
    assert [item["number"] for item in restored.json()["versions"]] == [1, 2, 3]


def test_restore_exposes_a_non_rewinding_revision_and_rejects_an_aba_write(tmp_path) -> None:
    client, _ = make_client(tmp_path)
    first = generate(client).json()
    manual = client.put(
        "/api/reports/2026-08-05",
        json={"sections": sections("第二版"), "expected_revision": first["revision"]},
    ).json()
    restored = client.post(
        "/api/reports/2026-08-05/versions/1/restore",
    ...
    assert restored.json()["current_version"] == 1
    assert restored.json()["revision"] > manual["revision"]
    stale_write = client.put(
        "/api/reports/2026-08-05",
        json={"sections": sections("旧页面内容"), "expected_revision": first["revision"]},
    )

    assert stale_write.status_code == 409
    assert stale_write.json()["detail"]["code"] == "report_version_conflict"
