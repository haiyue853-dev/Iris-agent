...
def test_concurrent_writers_cannot_both_commit_same_expected_version(tmp_path) -> None:
    ...
    first = threading.Thread(target=save_candidate, args=(report_with_versions(numbers=(1, 2)),))
    second = threading.Thread(target=save_candidate, args=(report_with_versions(numbers=(1, 3)),))
    first.start()
    second.start()
    first.join()
    second.join()
