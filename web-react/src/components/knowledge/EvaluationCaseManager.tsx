...
function isAnnotated(item: KnowledgeEvaluationCase): boolean {
  return Boolean(typeof item.expected_answerable === 'boolean' || item.expected_title || item.expected_document_id || item.relevant_titles?.length || item.relevant_document_ids?.length || item.relevant_chunk_ids?.length);
}
...
export function EvaluationCaseManager({ cases, validation, onChange, onLabel, onLabelMany, onValidate, onRun, validating = false, running = false }: Props) {
  ...
  return <section className="knowledge-evaluation-manager" aria-label="评测集管理">
    ...
    {cases.length ? <div className="knowledge-evaluation-table-wrap"><table><thead><tr><th>#</th><th>问题</th><th>预期标题</th><th>相关切片 ID</th><th>可回答性</th><th>状态</th><th>操作</th></tr></thead><tbody>{cases.map((item, index) => {
      ...
      return <tr key={`${index}-${item.question}`}><td>{index + 1}</td><td><input aria-label={`问题 ${index + 1}`} value={item.question} onChange={(event) => update(index, { question: event.target.value })} /></td><td><input aria-label={`预期标题 ${index + 1}`} value={item.expected_title || ''} onChange={(event) => update(index, { expected_title: event.target.value || undefined })} /></td><td><input aria-label={`相关切片 ${index + 1}`} value={(item.relevant_chunk_ids || []).join(',')} onChange={(event) => update(index, { relevant_chunk_ids: event.target.value.split(',').map((value) => value.trim()).filter(Boolean) })} /></td><td><select aria-label={`预期可回答性 ${index + 1}`} value={typeof item.expected_answerable === 'boolean' ? String(item.expected_answerable) : ''} onChange={(event) => update(index, { expected_answerable: event.target.value === '' ? undefined : event.target.value === 'true' })}><option value="">未标注</option><option value="true">有答案</option><option value="false">无答案</option></select></td><td><div className="knowledge-evaluation-case-status">{duplicate && <span>重复问题</span>}{empty && <span>空标注</span>}{invalidIds.length > 0 && <span>失效切片：{invalidIds.join(', ')}</span>}{!duplicate && !empty && !invalidIds.length && <span className="ready">已标注</span>}</div></td><td><div className="knowledge-evaluation-case-actions"><button type="button" aria-label={`标注问题 ${index + 1}`} onClick={() => onLabel(item.question)}>标注</button><button type="button" aria-label={`删除问题 ${index + 1}`} onClick={() => onChange(cases.filter((_, rowIndex) => rowIndex !== index))}>删除</button></div></td></tr>;
