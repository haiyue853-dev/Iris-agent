...
import { ReportApiError, ensureReportWorkspace, generateReport, listReports } from './reports';
...
describe('reports API', () => {
  ...
  it('ensures an empty workspace draft for the selected date', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      new Response(JSON.stringify(report), { status: 200 }),
    ));

    await expect(ensureReportWorkspace('2026-08-05')).resolves.toEqual(report);
    expect(fetch).toHaveBeenCalledWith(
      'http://localhost:8000/api/reports/2026-08-05/workspace',
      expect.objectContaining({ method: 'POST' }),
    );
  });
