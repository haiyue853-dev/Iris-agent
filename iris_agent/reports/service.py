...
class DailyReportService:
    ...
    def generate(
        ...
        if existing is None:
            ...
            if expected_version != existing.revision:
                raise ReportVersionConflictError("日报已被其他操作更新，请刷新后重试")
        ...
        report = DailyReport.create(
            ...
            updated_at=now,
            attachments=existing.attachments if existing is not None else (),
    ...
    def ensure_workspace(self, report_date: str) -> DailyReport:
        with self.repository.report_lock(report_date):
            ...
            self.repository.save(report, expected_version=0)
            return self.repository.get(report_date)
    ...
    def restore(
        ...
        report = DailyReport.create(
            ...
            source_chat_snapshot=current.source_chat_snapshot,
            versions=current.versions,
            current_version=target.number,
    ...
    def update_attachments(
        ...
        with self.repository.report_lock(report_date):
            current = self._expected_report(report_date, expected_revision)
            updated = DailyReport.create(
                report_date=current.date,
                source_notes=current.source_notes,
                source_session_id=current.source_session_id,
                source_chat_snapshot=current.source_chat_snapshot,
            ...
            self.repository.save(updated, expected_revision=expected_revision)
        return self.repository.get(report_date)
