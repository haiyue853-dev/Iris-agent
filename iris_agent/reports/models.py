...
class DailyReport:
    ...
    def create(
        ...
        resolved_revision = max((item.number for item in version_list), default=1) if revision is None else revision
        if not isinstance(resolved_revision, int) or isinstance(resolved_revision, bool) or resolved_revision < 1:
            raise ReportValidationError("日报修订号必须大于 0")
        report = cls(
            date=report_date,
