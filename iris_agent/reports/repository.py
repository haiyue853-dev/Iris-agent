...
class JsonDailyReportRepository:
    ...
    def save(
        ...
        *,
        ...
        expected = expected_revision if expected_revision is not None else expected_version
        with self._lock_for(report.date):
            ...
            if path.exists():
                ...
                if report.revision != current.revision + 1:
                    raise ReportVersionConflictError("日报已被其他操作更新，请刷新后重试")
    ...
    def _decode(raw: dict[str, object]) -> DailyReport:
        ...
        if not isinstance(attachments_raw, list):
            raise ReportValidationError("日报附件字段无效")
        attachments = tuple(
            ReportAttachment(
                id=item["id"],
                ...
                extraction_message=item.get("extraction_message"),
            )
        ...
        return DailyReport.create(
            ...
            revision=int(raw.get("revision", max((item.number for item in versions), default=1))),
        )
