...
class McpCenterService:
    ...
    def ensure_builtin_interview_server(self, knowledge_path: Path) -> McpServer:
        ...
        current = self._servers.get(server.id)
        if current != server:
    ...
    def set_enabled(self, server_id: str, enabled: bool) -> McpServer:
        ...
        updated = McpServer(current.id, current.name, current.command, current.args, current.allowed_tools, bool(enabled), current.environment, current.timeout_seconds)
    ...
    def set_allowed_tools(self, server_id: str, allowed_tools: tuple[str, ...]) -> McpServer:
        if not isinstance(allowed_tools, tuple) or not all(isinstance(item, str) and item.strip() for item in allowed_tools):
            raise ValueError("allowed_tools are invalid")
        current = self.get(server_id)
        updated = McpServer(current.id, current.name, current.command, current.args, tuple(dict.fromkeys(allowed_tools)), current.enabled, current.environment, current.timeout_seconds)
        self._servers[server_id] = updated
        self._save()
        return updated
    ...
    def set_environment(self, server_id: str, environment: dict[str, str]) -> McpServer:
        ...
        if not all(isinstance(key, str) and key.replace("_", "a").isalnum() and key[:1].isalpha() and len(key) <= 100 and isinstance(value, str) and "\x00" not in value and len(value) <= 10_000 for key, value in environment.items()):
            raise ValueError("environment is invalid")
        current = self.get(server_id)
        updated = McpServer(current.id, current.name, current.command, current.args, current.allowed_tools, current.enabled, tuple(sorted(environment.items())), current.timeout_seconds)
        self._servers[server_id] = updated
        self._close_session(server_id)
        self._save()
        return updated
    ...
    def set_timeout_seconds(self, server_id: str, timeout_seconds: int) -> McpServer:
        ...
        current = self.get(server_id)
        updated = McpServer(current.id, current.name, current.command, current.args, current.allowed_tools, current.enabled, current.environment, timeout_seconds)
        self._servers[server_id] = updated
        self._close_session(server_id)
        self._save()
        return updated
    ...
    def _load(self) -> dict[str, McpServer]:
        ...
        try:
            ...
            return {
                item["id"]: McpServer(item["id"], item["name"], item["command"], tuple(item["args"]), tuple(item["allowed_tools"]), bool(item.get("enabled", False)), tuple(sorted((key, value) for key, value in item.get("environment", {}).items() if isinstance(key, str) and isinstance(value, str))), item.get("timeout_seconds", 10))
    ...
    def _save(self) -> None:
        ...
        payload = {"servers": [
            {"id": item.id, "name": item.name, "command": item.command, "args": list(item.args), "allowed_tools": list(item.allowed_tools), "enabled": item.enabled, "environment": dict(item.environment), "timeout_seconds": item.timeout_seconds}
